...
export const ActiveMountsTable: React.FC<ActiveMountsTableProps> = ({ mounts, onUnmount }) => {
  ...
  return (
    <div className="glass-panel" style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: '1.2rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 10 }}>
            <Activity size={22} color="var(--neon-green)" />
      ...
      {mounts.length === 0 ? (
        <div style={{ padding: 48, textAlign: 'center', background: 'rgba(0,0,0,0.2)', borderRadius: 12, border: '1px dashed var(--glass-border)' }}>
          <HardDrive size={40} color="var(--text-dim)" style={{ marginBottom: 12 }} />
            ...
            <tbody>
              {mounts.map((m) => (
                <tr
                  ...
                  style={{ background: 'rgba(255,255,255,0.02)', transition: 'all 0.2s ease' }}
                >
                  <td style={{ padding: '14px 16px', borderTopLeftRadius: 8, borderBottomLeftRadius: 8 }}>
                    <div className="mono" style={{ fontSize: '1.2rem', fontWeight: 700, color: 'var(--neon-cyan)', display: 'flex', alignItems: 'center', gap: 8 }}>
                      <HardDrive size={18} />
                  ...
                  <td className="mono" style={{ padding: '14px 16px', fontSize: '0.8rem' }}>
                    <span style={{ color: 'var(--neon-green)' }}>↓ {formatBytes(m.bytes_read)}</span>
                    <span style={{ color: 'var(--neon-purple)', marginLeft: 8 }}>↑ {formatBytes(m.bytes_written)}</span>
                  </td>

                  <td style={{ padding: '14px 16px', textAlign: 'right', borderTopRightRadius: 8, borderBottomRightRadius: 8 }}>
                    <button
